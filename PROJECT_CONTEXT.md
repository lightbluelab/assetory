...
- `src/js/core.js`：公共状态、数据模型、严格校验与基础计算。
- `src/js/storage.js`：IndexedDB、加密、文件读写、导入导出和账本管理。
- `src/js/transactions.js`：流水联动、成本基础、对账和跨月同步。
- `src/js/quotes.js`：汇率与股价更新。
- `src/js/trends.js`：盈亏归因和趋势图表。
- `src/js/ui.js`：月度页面、资产/流水表和对话框。
- `src/js/app.js`：全局事件、PWA 和启动流程。
- `scripts/build.mjs`：将模板、CSS 和上述模块合并为独立 HTML 构建产物；无第三方依赖。
- `assetory.html`：独立账本工作台生成物，不应直接编辑。
- `index.html`：产品介绍页生成物，不应直接编辑。
- `assetory-demo-ledger.json`：公开演示账本；运行时由网页加载，**不得内嵌到 HTML 构建产物**。
- `manifest.webmanifest` / `service-worker.js`：PWA 安装与离线程序缓存；修改缓存资源或加载模型时递增 `CACHE_NAME`。
- `README.md`：面向使用者的两种使用方式与最小开发者入口。

修改源码后必须运行 `npm run build`，并用 `npm run check` 确认 `index.html`、`assetory.html` 和 `guide.html` 未过期。如需部署，可使用任意支持 HTTPS 的静态网站服务。发布时只上传公开程序与演示文件，绝不上传真实账本 JSON、备份或密钥。

## 数据模型

```text
ledger.schema = "assetory-ledger-v1"
ledger.months[YYYY-MM]
  balance[]       月末资产/负债；每项必须有稳定的 id
  flows[]         流水；通过 fromAssetId / toAssetId 引用资产 id
  fxRates         本月原币 -> CNY 汇率
  opening[]       必填的显式期初快照，用于趋势、同步和对账
...
2. `validateLedger()` 与 `normalizeLedger()` 只接受最新的 `assetory-ledger-v1`，分别负责严格结构校验与无语义损失的文本/数值规范化，不推断或修复旧账本和外部非规范数据。任何用户文本渲染到 `innerHTML` 前都用 `escapeHTML` / `escapeAttr`。
3. 流水新增或编辑必须先通过 `buildFlowDraft()` 校验，再写入并调用 `applyFlow()`；删除流水必须以 `applyFlow(flow, -1)` 回滚。新流水的金额和数量必须为正数。股票允许从零持仓建立空头，但单笔买卖不得穿过零仓，反向建仓必须拆成两笔；基金和固定资产不可超卖；还款不得超过当前负债余额；跨币种还款必须先通过现金转账换汇。收入可流入任意非负债资产；流入股票时以数量和价格联动持仓，其余资产按金额联动。只有流入现金资产的收入计入净现金流，股票或实物激励等非现金收入只增加资产与净资产。
...
7. 本月资产盈亏与收入/支出的净资产贡献不同：资产盈亏用于价格、汇率、买卖成本和资产收益归因；收入/支出在盈亏趋势中单列。归因、净资产变动和月面板环比统一以上一个实际月份的期末为比较基准；账本首月使用必填的 `opening` 显式期初快照。`opening` 同时是趋势、同步重建和对账的唯一首月基准，不再从期末反向推断。现金盈亏另外拆分汇率影响、资产收益和手工余额调整。
8. 所有修改应由 `transact()` 自动生成字段级变更明细；同步提示展示余额、数量、价格、汇率和流水去向等具体差异。不得依赖时间戳判断是否需要同步。
9. JSON 导入只接受规范 `assetory-ledger-v1` 数据。收入流水必须显式包含布尔值 `nonCashIncome` 和 `incomeAssetMode: "quantity" | "value"`，不得根据 `qty`、`price` 或资产类别猜测历史语义。
10. `src/` 是唯一源码，根目录的 `index.html`、`assetory.html` 和 `guide.html` 都是构建产物。所有功能修改必须落在对应模块中，再运行构建脚本；禁止只修改生成物，否则下一次构建会覆盖变更。
11. 在线示例由 `loadDemoLedger()` 通过相对路径请求 `assetory-demo-ledger.json`；构建脚本不得读取、拼接或内嵌该 JSON。Service Worker 必须缓存该独立资源，保证已安装 PWA 的离线示例可用。
